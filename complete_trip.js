...
const TripTable = ({ employeeId }) => {
    ...
    useEffect(() => {
        const fetchTrips = async () => {
            try {
                const response = await fetch(`http://192.168.1.3/UK_UserLogin/complete-trip.php?employee_id=${employeeId}`);
    ...
    useEffect(() => {
        const fetchAdditionalDetails = async () => {
            try {
                const response = await fetch(`http://192.168.1.3/UK_UserLogin/drop_complete_trip.php?employee_id=${employeeId}`);
                if (response.ok) {
                    const data = await response.json();
    ...
    return (
        <ScrollView style={styles.container}>
            <View>
                <Text style={styles.sectionTitle}>Completed Trips Table</Text>
                {loading ? (
            ...
            <View>
                <Text style={styles.sectionTitle}>Drop Completed Details</Text>
                        ...
                        <ScrollView horizontal>
                            <View style={styles.table}>
                                <View style={[styles.row, styles.header]}>
                                    ...
                                    <Text style={[styles.cellLabel, styles.borderRight]}>Driver Name</Text>
                                    <Text style={[styles.cellLabel, styles.borderRight]}>Cab No</Text>
                                    <Text style={[styles.cellLabel, styles.borderRight]}>Drop Timings</Text>
                                    <Text style={[styles.cellLabel, styles.borderRight]}>Drop Date</Text>
                                    <Text style={styles.cellLabel}>Out Timing</Text>
                                </View>
                                {additionalDetails.map((detail, index) => (
                                    <View key={index} style={[styles.row, styles.borderTop]}>
                                        <Text style={[styles.cellValue, styles.borderRight]}>{detail.employee_id}</Text>
                                        <Text style={[styles.cellValue, styles.borderRight]}>{detail.driver_name}</Text>
                                        <Text style={[styles.cellValue, styles.borderRight]}>{detail.cab_no}</Text>
                                        <Text style={[styles.cellValue, styles.borderRight]}>{detail.drop_timings}</Text>
                                        <Text style={[styles.cellValue, styles.borderRight]}>{detail.drop_date}</Text>
